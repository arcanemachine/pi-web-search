        ...
        async execute(_toolCallId, params, _onUpdate, _ctx, _signal) {
            ...
            if (contentType.includes("text/html") || url.endsWith(".html")) {
                ...
                let text = html
                    ...
                    .replace(/<[^>]+>/g, " ")
                    .replace(/\n\s*\n/g, "\n\n");
                return { content: [{ type: "text", text: grepWithContext(text, query) }], details: { url, query } };
            }
            // Handle plain text or other content types directly
            const textContent = await res.text();
            return { content: [{ type: "text", text: grepWithContext(textContent, query) }], details: { url, query } };
        },
    });
}
function grepWithContext(text, query) {
