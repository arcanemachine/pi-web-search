...
Phase 0, Phase 1, and Phase 2 are implemented and verified. `index.ts` is a thin registrar for all three focused tools. The public `search_web` tool is search-only, uses the configured `ddgr`/SearXNG order, returns bounded structured outcomes and provenance, distinguishes evidence-backed operational failures from legitimate emptiness, enforces the process-local limiter, and uses bounded TTL/LRU caching plus identical in-flight coalescing. Both adapters enforce timeout/cancellation and validate native data; `ddgr` uses argv-only execution and a cached version/PATH probe.

The shared document service now enforces HTTP(S), credential, redirect, timeout, cancellation, download, normalized-byte, line-index, cache-entry, and cache-byte bounds. It normalizes static HTML with `linkedom` and `node-html-markdown`, preserves supported native text/Markdown/JSON, records source metadata and hashes, coalesces in-flight fetches, and caches only successful snapshots. Authenticated opaque cursors bind operation/options/position to the exact cached snapshot. `read_url_content` pages on Unicode character boundaries, while `grep_url_content` performs bounded literal matching with exact snapshot-reproducible quotes, heading/line/character locators, overlapping-window coalescing, explicit `no_match`, and stable pagination.
